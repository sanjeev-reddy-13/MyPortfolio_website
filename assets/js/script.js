...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const certLinks = document.querySelectorAll('[data-page="certifications"] a[data-certificate-img]');
  ...
  const flightProject = Array.from(document.querySelectorAll('[data-page="projects"] .project-item'))
    .find(item => {
  ...
  document.querySelectorAll('[data-page="projects"] a.project-details-trigger[data-project-id], [data-page="projects"] a.project-description-btn[data-project-id]').forEach(projectLink => {
    projectLink.addEventListener('click', function (e) {
      e.preventDefault();

      const projectId = this.getAttribute('data-project-id');
      const projectCard = this.closest('.project-item');
      const projectTitle = projectCard?.querySelector('.project-title')?.textContent?.trim() || projectId || 'Project Details';
      document.getElementById('project-details-modal-body').innerHTML = `
        <div>
          <h2>${projectTitle}</h2>
          <div class="project-section">
            <div class="project-section-title"><span>Project Description</span></div>
            <div class="project-overview">
              <p>Detailed project description will be added here later.</p>
            </div>
          </div>
        </div>
      `;
  ...
  document.querySelectorAll('.project-github-btn').forEach(button => {
